...
class associaNoticiaCategoriaController {
            ...
            this.findMysqlNoticia(noticia, async noticia_cb => {
                ...
                if(noticia_cb.length > 0){
                    ...
                    try {
                        ...
                        //Já tenho o objeto de categories continuar daqui carlos marinho 
                        //descomentar linhas abaixo e continuar 15/09/18

                        //let img = await this.updateStrypeAssociacao(jwt.data.jwt, noticia._id, obj);
    ...
    async insertStrypeAssociacao(jwt, image){
    ...
    async getNoticias(jwt){
        ...
        //console.log("\n\nconfig: ", config);
        try{
            let ret = await axios.get('http://localhost:1337/noticia?_start=0&_limit=2',  config);
            return ret;
        }
        catch(e){
            console.log("\n\n\n error: ", e);
    ...
    findMysqlNoticia(noticia, cb){
        ...
        let sql = ` select p.ID, p.post_title, t.term_id, t.name, t.slug, tt.taxonomy from nkty_posts p
